...
def upadtephagecsv_cluster(userpath):
    outputpath = userpath+'/output/'
    phagepath = outputpath+'result/phage.tsv'
    clusterpath = outputpath+'rawdata/comparedatabse/result.txt'
    f_result = open(clusterpath,'r')
    cluster_dict = {}
    subcluster_dict = {}
    source_dict={}
    for line in f_result:
        ...
        subcluster_dict[line[0]]=line[2]
        source_dict[line[0]]=line[3]
    f_result.close()
    phage = pd.read_csv(phagepath, sep='\t', index_col=False)
    phage['cluster']=phage['Acession_ID'].map(cluster_dict)
    phage['subcluster']=phage['Acession_ID'].map(subcluster_dict)
    phage['source']=phage['Acession_ID'].map(source_dict)
    phage.to_csv(phagepath, sep='\t', index=False)
