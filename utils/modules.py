...
def cluster(taskpath):
    ...
    try :    
        with open(clusterpath,'r') as resultf:
            result = resultf.read()
        clusters = pd.read_csv(clusterpath, sep='\t', index_col=False,names=[
                            'phage_id', 'cluster','subcluster']).astype(str)
    except:
        clusterpath = taskpath+'/output/rawdata/comparedatabse/result.txt'
        clusters = pd.read_csv(clusterpath, sep='\t', index_col=False,names=[
                            'phage_id', 'cluster','subcluster','Source']).astype(str)
...
def alignmentdetail(taskpath,cid=None,pids=None):
    ...
    with open(sortedlistpath, 'r') as f:
        ...
        for line in sortedlistlines:
            ...
            order+=1
    circlealignments=pd.read_csv(aligncirclepath, sep=',', index_col=False,nrows=500000,names=['id','Source_Phage_ID','Target_Phage_ID',
