...
def add_data():
    ...
    path='/home/platform/phage_db/phage_api/workspace/csv_data/Download/Annotated_protein_meta_data_v2/temphd_phage_annotated_protein_meta_data.tsv'
    proteincsv=pd.read_csv(path,sep='\t',low_memory=False)
    proinfodict={}
    for row in proteincsv.itertuples():
        proinfodict[row[7]]=[row[3],row[8],row[9]]
    from phage_protein.models import phage_protein_TemPhD
    proteins = phage_protein_TemPhD.objects.order_by('id')
    print("loading data")
    count=0
