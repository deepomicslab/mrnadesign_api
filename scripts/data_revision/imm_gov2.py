...
def add_data():
    ...
    path='/home/platform/phage_db/phage_data/data/csv_data/Download/Transmembrane_protein_meta_data/gov2_phage_transmembrane_protein_meta_data.tsv'
    phage_protein_tmm = pd.read_csv(path, sep=',')
    print('load data')
    for row in phage_protein_tmm.itertuples():
        phage_protein_tmhmm_GOV2.objects.create(Phage_Acession_ID=row.Phage_ID, Length=row.Length, predictedTMHsNumber=row.PredictedTMHsNumber, ExpnumberofAAsinTMHs=row.ExpnumberofAAsinTMHs, 
