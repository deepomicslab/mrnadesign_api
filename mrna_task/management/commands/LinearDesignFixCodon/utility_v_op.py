...
def func1(sequence: str, hairpin_type: int, params) -> float:
    """
    获取特定发夹环类型的能量。
...
def func3(i: int,
    ...
    若是，则返回 params['TerminalAU37'][0]；否则返回 0。
    ...
    if nuci == 4:  # 左侧是 U
        ...
        if (nuci not in (1, 3)) or (nucj_1 != 4):
            return 0
    return params['TerminalAU37'][0]

# def func4(a: int, b: int, c: int, d: int, e: int, f: int, g: int) -> int:
#     return 0.0

def func4(a1, a2, n5d, base_i, base_j, n3d, params):
    """
    ...
    if n5d_valid and n3d_valid:
        # 使用 params['mismatchExt37']
        # IDA 中见到地址：100*v6 + 20*n5d + 4*n3d  => 说明每一层嵌套的乘法
        index = (v6 * 25) + (n5d * 5) + n3d
        temp_energy = params['mismatchExt37'][index]
    elif n5d_valid and (not n3d_valid):
        # 使用 params['dangle5_37']
        index = (v6 * 5) + n5d
        temp_energy = params['dangle5_37'][index]
    elif (not n5d_valid) and n3d_valid:
        # 使用 params['dangle3_37']
        index = (v6 * 5) + n3d
        temp_energy = params['dangle3_37'][index]
    else:
        # 两侧都无效
        temp_energy = 0

    # 第 3 步：如果 v6>2，则加 params['TerminalAU37'][0]
    add_terminal = params['TerminalAU37'][0] if (v6 > 2) else 0
...
#     return 0.0
def func5(py_type: int, b: int, c: int, params) -> int:
    """
    ...
    """
    if py_type <= 2:
        # 不额外加 params['TerminalAU37'][0]
        return params['ML_intern37'][0]
    else:
        # 额外加 params['TerminalAU37'][0]
        return params['ML_intern37'][0] + params['TerminalAU37'][0]
...
def func6(i: int,
    ...
    判断 (nuci, nucj_1) 是否为 AU、UA、GU、UG 配对：
      - 若是，则返回 params['TerminalAU37'][0] + ML_intern37[0]
      - 否则只返回 ML_intern37[0]
    ...
    return params['TerminalAU37'][0] + params['ML_intern37'][0]
...
def func7(   param1: int,
    ...
    Python 版，对应逆向 C 函数 func7 的逻辑：
      ...
      3. 若 pairType>2 则加 params['TerminalAU37'][0]
    ...
    if (neighbor5 >= 0) and (neighbor3 >= 0):
        ...
        mismatchVal = params['mismatchM37'][index]
    elif (neighbor5 >= 0):
        index = pairType * 5 + neighbor5
        mismatchVal = params['dangle5_37'][index]
    elif (neighbor3 >= 0):
        index = pairType * 5 + neighbor3
        mismatchVal = params['dangle3_37'][index]
    else:
        mismatchVal = 0

    # --- 3) params['TerminalAU37'][0] 或 0 ---
    if pairType > 2:
        termAU = params['TerminalAU37'][0]
    else:
        termAU = 0

    # --- 4) 返回计算结果 ---
    result = params['ML_closing37'][0] + params['ML_intern37'][0] + mismatchVal + termAU
    return result
...
def func10(pairType: int, neighbor5: int, neighbor3: int, params) -> int:
    """
    Python版，与C版func10对应。
    模拟multibranch stem能量计算：
      - mismatch/dangle
      - optional params['TerminalAU37'][0]
      - + params['ML_intern37'][0]
    ...
    if (neighbor5 >= 0) and (neighbor3 >= 0):
        index = (pairType * 25) + (neighbor5 * 5) + neighbor3
        partialEnergy = params['mismatchM37'][index]
    elif (neighbor5 >= 0):
        index = pairType * 5 + neighbor5
        partialEnergy = params['dangle5_37'][index]
    elif (neighbor3 >= 0):
        index = pairType * 5 + neighbor3
        partialEnergy = params['dangle3_37'][index]
    else:
        partialEnergy = 0  # both sides invalid => 0

    # --- 2) 根据 pairType>2 与否 => params['TerminalAU37'][0] or 0
    ...
    # --- 3) 返回 partialEnergy + termAU + params['ML_intern37'][0]
    return partialEnergy + termAU + params['ML_intern37'][0]
...
def func11(a1: int, a2: int, a3: int,
    ...
    return mismatchVal + termAU + params['ML_intern37'][0]

# def func12(a: int, b: int, c: int, d: int, e: int, f: int, g: int, h: int) -> int:
#     return 0.0
def func12(i, jnext, nuci, nuci1, nucjnext_1, nucjnext, tetra_hex_tri, params):
    """
    ...
    if hairpin_length >= 31:
        ...
        energy_result = params['hairpin37'][30] + int(params['lxc37'][0] * log_input)

        # Calculate mismatch energy
        mismatch_index = 25 * energy_category + 5 * nuci1 + nucjnext_1
        mismatch_energy = params['mismatchH37'][mismatch_index]
        final_energy = mismatch_energy + energy_result

        return final_energy
    else:
        # Get hairpin energy from params['hairpin37'] table
        energy_result = params['hairpin37'][hairpin_length]
        if hairpin_length >= 3:
            if hairpin_length == 4 and (tetra_hex_tri & 0x80000000) == 0:
                return params['Tetraloop37'][tetra_hex_tri]  # Specific tetraloop energy
            if hairpin_length == 6 and (tetra_hex_tri & 0x80000000) == 0:
                return params['Hexaloop37'][tetra_hex_tri]     # Specific hexaloop energy
            if hairpin_length == 3:
                if (tetra_hex_tri & 0x80000000) == 0:
                    return params['Triloop37'][tetra_hex_tri]   # Specific triloop energy
                else:
                    TermiAU = params['TerminalAU37'][0] if energy_category > 2 else 0
...
def func14(
    ...
    if loop_length_long == 0:
        return params['stack37'][8 * energy_category_closing + energy_category_enclosed]
    ...
    if loop_length_short == 0:
        if loop_length_long <= MAXLOOP:
            energy = params['bulge37'][loop_length_long]
        else:
            energy = params['bulge37'][30] + int(params['lxc37'][0] * math.log(loop_length_long / 30.0))

        if loop_length_long == 1:
            # 堆叠
            energy += params['stack37'][8 * energy_category_closing + energy_category_enclosed]
        else:
            # TerminalAU
            if energy_category_closing > 2:
                energy += params['TerminalAU37'][0]
    ...
    # --------------------------
        ...
        if loop_length_long == 1:
            ...
            if (loop_length_long + 1) <= MAXLOOP:
                energy = params['internal_loop37'][loop_length_long + 1]
            else:
                energy = (params['internal_loop37'][30] +
                          int(params['lxc37'][0] * math.log((loop_length_long + 1) / 30.0)))

            energy += MIN2(params['MAX_NINIO'][0], (loop_length_long - loop_length_short) * params['ninio37'][0])
            # mismatch1nI
            energy += (params['mismatch1nI37'][energy_category_closing * 25 + 5 * nucp + nucq_1] +
                       params['mismatch1nI37'][energy_category_enclosed * 25 + 5 * nucj + nuci_1])
    ...
    # --------------------------
        ...
        if loop_length_long == 2:
            ...
            index = (5000 * energy_category_closing +
                     ...
                     125 * nucp + 25 * nuci_1 +
                     5 * nucj + nucq_1)
            energy = params['int22_37'][index]
        elif loop_length_long == 3:
            energy = params['internal_loop37'][5] + params['ninio37'][0]
            energy += (params['mismatch23I37'][25 * energy_category_closing + 5 * nucp + nucq_1] +
                       params['mismatch23I37'][25 * energy_category_enclosed + 5 * nucj + nuci_1])
        else:
            # 通用内部环
            total_loop_length = loop_length_long + loop_length_short
            if total_loop_length <= MAXLOOP:
                energy += params['internal_loop37'][total_loop_length]
            else:
                energy += (params['internal_loop37'][30] +
                           int(params['lxc37'][0] * math.log(total_loop_length / 30.0)))
            energy += MIN2(params['MAX_NINIO'][0], (loop_length_long - loop_length_short) * params['ninio37'][0])
            energy += (params['mismatchI37'][25 * energy_category_closing + 5 * nucp + nucq_1] +
                       params['mismatchI37'][25 * energy_category_enclosed + 5 * nucj + nuci_1])
    ...
    # --------------------------
        ...
        # if temp1 > len(params['mismatchI37']) -1:
        #     temp1 = len(params['mismatchI37']) -1
        # if temp2 > len(params['mismatchI37']) - 1:
        #     temp2 = len(params['mismatchI37']) -1
        energy += (params['mismatchI37'][temp1] + params['mismatchI37'][temp2])

    return energy


def func15(i, j, nuci, dummy1, dummy2, nucj_1, seq_length, params):
    """
    ...
    if nucj_1 != 4:
        ...
        if (nucj_1 in [1, 3]) and (nuci == 4):
            add_terminal = params['TerminalAU37'][0]
    else:
        # nucj_1 == 4 的情况
        if (nuci | 2) == 3:
            add_terminal = params['TerminalAU37'][0]

    result = params['ML_closing37'][0] + params['ML_intern37'][0] + add_terminal
    return result
...
#
#     params['Hexaloop37'] = [-7.0, -8.5, -9.0]
#     params['Tetraloop37'] = [-5.0, -5.5, -6.0]
#     params['Triloop37'] = [-3.0, -3.5, -4.0]
#
#     # 示例调用
#     loop_struct_example = {
#         'flag': 1,
